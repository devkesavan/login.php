...
<section class=" text-center text-lg-start">
  ...
  <div class="card mb-3">
    ...
    if (isset($_POST['submit']) && $_FILES['document']['size'] > 0) {		
       ...
       fileName = $_FILES['document1']['name'];
        ...
        $document $photo=file_get_contents($photo);
        $photo=base64_encode($photo);
        $ = base64_encode($document);
      
		
	
		$sqli = "INSERT INTO `register`(`name`, `d_o_b`, `gender`, `phone_no`, `email`, `user_pwd`, `qualification`, `skill`, `address`, `image`, `RESUME`) VALUES ('$full_name','$d_o_b','$gender','$phone_number','$email','$password','$qualification','".$skill."','$address','$photo','$document')";
    mysqli_query($conn, $sqli);
   $sqli2="INSERT INTO `users`(`user_name`, `user_pwd`) VALUES ('$full_name','$password')";
   mysqli_query($conn, $sqli2);
   
    echo "<script>alert('SUCESSFULLY REGISTER YOUR DETAIL');window.location.href='index.php';</script>"; 
